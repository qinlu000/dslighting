...
class AgenticDataBenchTask:
    """Validated, solver-facing subset of one upstream JSONL record."""
...
def write_selected_task_config(
    ...
    if not selected_ids or len(selected_ids) != len(task_ids):
        raise ValueError("Selected task ids must be non-empty and unique")
    ...
    with source.open("r", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            if not isinstance(payload, dict):
                raise ValueError(f"Expected JSON object on {source}:{line_number}")
            task_id = str(payload.get("id") or "")
            if task_id in selected_ids:
                selected[task_id] = payload

    missing = selected_ids.difference(selected)
    if missing:
        raise ValueError(f"Selected task ids missing from {source}: {sorted(missing)}")

    destination.parent.mkdir(parents=True, exist_ok=True)
    destination.write_text(
        "".join(
            json.dumps(selected[task_id], ensure_ascii=False) + "\n"
...
def select_tasks(
    ...
    *,
    ...
    try:
        ...
        raise ValueError(
            f"Invalid index expression {expression!r}; use '0-5' or '0,2,4'"
        ) from exc
...
def _plot_instructions(task: AgenticDataBenchTask) -> str:
    image_names = [
        name for name in task.output_file_names if Path(name).suffix.lower() == ".png"
    ]
...
def _io_instructions(task: AgenticDataBenchTask, output_dir: Path) -> str:
    ...
    return (
        ...
        "Each Python Action runs in a fresh process: repeat all imports and recreate "
        "in-memory variables in every Action. Files written to the workspace persist.\n"
    ...
    *,
    ...
    for source_item in source_dir.iterdir():
        ...
        if destination.exists() or destination.is_symlink():
            continue
        try:
            os.symlink(source_item.resolve(), destination, target_is_directory=source_item.is_dir())
...
def _load_messages(record: Mapping[str, Any] | None) -> list[dict[str, Any]]:
    ...
    if metadata_path.name:
        candidates.append(metadata_path.parents[1] / "messages.json")
    workspace_dir = Path(str(record.get("workspace_dir") or ""))
    if workspace_dir.name:
        candidates.append(workspace_dir / "artifacts" / "messages.json")
...
def _last_task_record(records: Iterable[Mapping[str, Any]], task_id: str) -> Mapping[str, Any] | None:
...
async def run_tasks(
    ...
    """Run selected tasks sequentially and write an experiment summary."""
    ...
    if settings.max_steps <= 0 or settings.timeout_seconds <= 0:
    ...
    # Match the other experiment entrypoints while preserving explicitly set
    ...
    config = ConfigBuilder().build_config(
        ...
        agent_runtime={"max_steps": settings.max_steps},
    )
    ...
    async def run_one(task: AgenticDataBenchTask) -> dict[str, Any]:
        ...
        print(f"run task={task.task_id} workflow={settings.workflow} model={settings.model}")
        try:
            ...
            print(
                f"{status} task={task.task_id} cost={cost:.6f} "
                f"missing={upstream['missing_files']}"
            )
    ...
    results = [
        result
        for _, result in sorted(indexed_results, key=lambda indexed: indexed[0])
    ]

"""Bridge exported AgenticDataBench trajectories to native DataCOPE SkillManager."""
...
if __package__:
    ...
    from .protocol import TRAJECTORIES_PER_TASK, validate_round_index
...
def validate_prediction_runs(predictions_dir: Path) -> set[str]:
    ...
    if len(run_dirs) != TRAJECTORIES_PER_TASK:
        raise ValueError(
            f"Expected {TRAJECTORIES_PER_TASK} sample directories, found {len(run_dirs)}"
        )
    ...
    for run_dir in run_dirs:
        ...
        for path in sorted(run_dir.glob("prediction_*.json")):
            try:
                ...
                if not isinstance(payload["prediction"], str):
                    raise TypeError("prediction must be a string")
                query_ids.append(str(payload["extra_info"]["query_id"]))
            except (OSError, json.JSONDecodeError, KeyError, TypeError) as exc:
                raise ValueError(f"Invalid DataCOPE prediction {path}: {exc}") from exc
    ...
    if any(query_ids != expected for query_ids in query_sets[1:]):
        raise ValueError("AgenticDataBench sample runs contain different task sets")
    return expected
...
def _run(args: argparse.Namespace) -> dict[str, str]:
    ...
    writer_prompt = "\n\n".join(
        (
            ...
            f"The final AgenticDataBench Skill file must be saved at exactly `{skill_file}`.",
        )
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"DataCOPE AgenticDataBench bridge failed: {exc}", file=sys.stderr)
        return 1
